...
router.post('/verify', async (req, res) => {
  ...
  try {
    const result = await pool.query(
      'SELECT id,title,description,exam_type,university,department,section_filter,duration_minutes,start_time,end_time,total_questions,status,aptitude_time_minutes,verbal_time_minutes,device_allowed FROM exams WHERE room_code=$1',
