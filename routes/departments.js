...
// GET /api/departments/tree  — public, full tree (MUST be before /:id)
router.get('/tree', async (req, res) => {
  try {
    ...
    console.error('GET /departments/tree error:', err);
    res.status(500).json({ error: 'Server error' });
  }
});

// GET /api/departments/all  — all including inactive, admin only (MUST be before /:id)
...
// PATCH /api/departments/sections/:sectionId — MUST be before /:id
...
// DELETE /api/departments/sections/:sectionId — MUST be before /:id
...
// ── DYNAMIC /:id routes BELOW all static paths ───────────────────────────────

// GET /api/departments/:id/sections  — public
router.get('/:id/sections', async (req, res) => {
  try {
    ...
    console.error('GET /departments/:id/sections error:', err);
    res.status(500).json({ error: 'Server error' });
  }
});
